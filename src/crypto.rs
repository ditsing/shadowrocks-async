...
pub fn create_crypter(key_bytes: &[u8], nonce_type: NonceType, cipher_type: CipherType)
    -> Box<dyn Crypter> {
    if cipher_type == CipherType::Chacha20IetfPoly1305 {
        let crypter = sodium_crypter::Chacha20IetfPoly1305Crypter::create_crypter(
            key_bytes, nonce_type,
        );
        let spec = cipher_type.spec();
        assert_eq!(spec.key_size, sodium_crypter::Chacha20IetfPoly1305Crypter::KEY_BYTES);
        assert_eq!(spec.nonce_size, sodium_crypter::Chacha20IetfPoly1305Crypter::NONCE_BYTES);
    ...
    let cipher = match cipher_type {
        ...
        _ => {
            log::warn!("Unsupported cipher {:?}, falling back to aes-256-gcm", cipher_type);
    ...
    Box::new(openssl_crypter::OpensslCrypter::create(cipher, key_bytes, nonce_type))
}
...
pub fn derive_master_key_pbkdf2(password: &[u8], salt: &[u8], key_size: usize) -> Vec<u8> {
...
pub fn derive_master_key_compatible(password: &[u8], key_size: usize) -> Result<Vec<u8>> {
...
pub fn derive_subkey_compatible(master_key: &[u8], salt: &[u8], key_size: usize) -> Vec<u8> {
    let hkdf = hkdf::Hkdf::extract(Some(salt), master_key, hkdf::OpensslSha::sha1());
    hkdf.expand(SHADOW_INFO, key_size)
}

#[cfg(test)]
mod test {
    use crate::crypto::cipher_spec::{AES_128_GCM, AES_192_GCM, CHACHA20_IETF_POLY1305};
    ...
    fn test_size_assumptions() {
        for spec in &[&AES_128_GCM, &AES_192_GCM, &AES_256_GCM, &CHACHA20_IETF_POLY1305] {
