...
impl OpensslCrypter {
    ...
    pub fn encrypt(&mut self, data: &[u8]) -> Result<(Vec<u8>, [u8; TAG_BYTES])> {
    ...
    pub fn decrypt(&mut self, data: &[u8], tag: &[u8; TAG_BYTES]) -> Result<Vec<u8>> {
        // An empty aad is a no-op and should be handled gracefully by the library.
...
mod test {
    ...
    fn make_crypter(nonce_type: NonceType) -> OpensslCrypter {
        ...
        OpensslCrypter::create(Cipher::aes_256_gcm(), key.as_slice(), nonce_type)
    }
    ...
    fn test_encryption_decryption() -> Result<()> {
        ...
        let ciphertext = Crypter::encrypt(&mut encrypter, plaintext.as_slice())?;

        assert_eq!(
            ciphertext,
            [
                0xBD, 0x37, 0x9A, 0xDE, 0x77, 0x39, 0x01, 0xBD, 0x91, 0x7B, 0x90, 0x47, 0x73, 0x26,
                0xC3, 0x43, 0xB7, 0xC4, 0x9D, 0xB2, 0x87, 0x9D, 0x36, 0xF8, 0xCE, 0x26, 0x0C, 0xD7,
                0x0C, 0xD4, 0x6C, 0x19, 0x5C, 0xBA, 0xAF, 0xF9, 0x51, 0x13, 0xE4, 0xF3, 0x2D, 0xC0,
                0xA2, 0x59, 0x72, 0xEA, 0xF3, 0x95, 0x3D, 0x51, 0x1A, 0x05, 0xBA
            ]
            .to_vec(),
        );
        ...
        let decrypted_text = Crypter::decrypt(&mut decrypter, ciphertext.as_slice())?;
    ...
    fn test_multiple_encryption_decryption() -> Result<()> {
        ...
        let ciphertext1 = Crypter::encrypt(&mut encrypter, plaintext.as_slice())?;
        let ciphertext2 = Crypter::encrypt(&mut encrypter, plaintext.as_slice())?;

        assert_eq!(
            ciphertext1,
            [
                0xBC, 0x36, 0x9B, 0xDF, 0x76, 0x38, 0x00, 0xBC, 0x90, 0x7A, 0x91, 0x46, 0x72, 0x27,
                0xC2, 0x42, 0xB6, 0xC5, 0x9C, 0xB3, 0x86, 0x9C, 0x37, 0xF9, 0xCF, 0x27, 0x0D, 0xD6,
                0x0D, 0xD5, 0x6D, 0x18, 0x5D, 0xBB, 0xAE, 0xF8, 0x50, 0xB0, 0x3C, 0xA5, 0x92, 0x9D,
                0x26, 0xF2, 0x0F, 0x3A, 0x09, 0xE5, 0xF8, 0x6A, 0x40, 0x61, 0xF4
            ]
            .to_vec(),
        );
        assert_eq!(
            ciphertext2,
            [
                0x54, 0x94, 0x1F, 0x3B, 0x2B, 0x5D, 0xFC, 0x59, 0xD4, 0x26, 0xAE, 0xB1, 0xA5, 0x10,
                0xFF, 0x60, 0x32, 0x8F, 0xFB, 0xC5, 0xCB, 0x72, 0xD0, 0xED, 0xF5, 0xBA, 0xE5, 0x93,
                0xC3, 0x79, 0x5A, 0x33, 0x4F, 0x27, 0x2B, 0x94, 0x75, 0x50, 0x74, 0x22, 0x23, 0x03,
                0xF3, 0x59, 0x22, 0xAA, 0xE2, 0x7D, 0x97, 0x52, 0xB9, 0x84, 0x0E
            ]
            .to_vec(),
        );
        ...
        let mut decrypter = make_crypter(NonceType::Sequential);
        let decrypted_text1 = Crypter::decrypt(&mut decrypter, ciphertext1.as_slice())?;
        let decrypted_text2 = Crypter::decrypt(&mut decrypter, ciphertext2.as_slice())?;
    ...
    fn test_encryption_decryption_zero_nonce() -> Result<()> {
        ...
        assert_eq!(
            ciphertext1,
            [
                0xBB, 0x31, 0x9C, 0xD8, 0x71, 0x3F, 0x07, 0xBB, 0x97, 0x7D, 0x96, 0x41, 0x75, 0x20,
                0xC5, 0x45, 0xB1, 0xC2, 0x9B, 0xB4, 0x81, 0x9B, 0x30, 0xFE, 0xC8, 0x20, 0x0A, 0xD1,
                0x0A, 0xD2, 0x6A, 0x1F, 0x5A, 0xBC, 0xA9, 0xFF, 0x57, 0x5F, 0x35, 0x04, 0xAE, 0x0D,
                0xB9, 0xA2, 0x7E, 0x08, 0xEE, 0xB7, 0xA3, 0xCA, 0xC7, 0x5C, 0x1C
            ]
            .to_vec(),
        );
