...
impl Chacha20IetfPoly1305Crypter {
    ...
    pub fn create_crypter(key_bytes: &[u8], nonce_type: NonceType) -> Self {
        Self {
            key: chacha20poly1305_ietf::Key::from_slice(key_bytes).expect("Error creating crypter"),
            nonce: chacha20poly1305_ietf::Nonce::from_slice(&vec![0u8; Self::NONCE_BYTES])
...
impl Crypter for Chacha20IetfPoly1305Crypter {
    fn encrypt(&mut self, data: &[u8]) -> Result<Vec<u8>> {
        let ret = chacha20poly1305_ietf::seal(&data, None, &self.nonce, &self.key);
    ...
    fn decrypt(&mut self, data: &[u8]) -> Result<Vec<u8>> {
        let ret = chacha20poly1305_ietf::open(&data, None, &self.nonce, &self.key);
        match self.nonce_type {
            NonceType::Sequential => self.nonce.increment_le_inplace(),
            _ => (),
        }
...
mod test {
    ...
    fn test_encryption_decryption() -> Result<()> {
        ...
        assert_eq!(
            ciphertext1,
            [
                0x03, 0x25, 0x4F, 0xD3, 0xEC, 0x50, 0x14, 0xC0, 0x06, 0xF9, 0x6B, 0x8F, 0xAC, 0xA6,
                0x59, 0x50, 0xEB, 0x89, 0x6D, 0x0D, 0xDF, 0x06, 0x6F, 0xCE, 0x38, 0x84, 0xB9, 0x05,
                0x9C, 0x34, 0xF2, 0x26, 0x16, 0x7C, 0x49, 0x85, 0x5F, 0xCE, 0x8F, 0x0E, 0xDF, 0xA0,
                0x26, 0x56, 0x6B, 0x5B, 0xF9, 0x3C, 0x54, 0xB6, 0x4E, 0x10, 0x4A
            ]
            .to_vec(),
        );
        assert_eq!(
            ciphertext2,
            [
                0x00, 0x51, 0xAC, 0x62, 0x34, 0x4A, 0x28, 0xE7, 0x5B, 0xCF, 0x21, 0x58, 0x1B, 0xFB,
                0xD1, 0x35, 0x86, 0x8F, 0x7B, 0xC3, 0x1E, 0x5C, 0xAE, 0x08, 0x4B, 0x8B, 0x21, 0x1E,
                0x44, 0xDE, 0xAD, 0xCD, 0x13, 0x72, 0xB0, 0xC2, 0xB6, 0x09, 0x99, 0x9D, 0xB3, 0xD4,
                0xB9, 0x34, 0xFC, 0x44, 0x34, 0xFF, 0x3B, 0xEF, 0x58, 0x71, 0xD3
            ]
            .to_vec(),
        );
    ...
    fn test_encryption_decryption_zero_nonce() -> Result<()> {
        ...
        assert_eq!(
            ciphertext1,
            [
                0x02, 0x24, 0x4E, 0xD2, 0xED, 0x51, 0x15, 0xC1, 0x07, 0xF8, 0x6A, 0x8E, 0xAD, 0xA7,
                0x58, 0x51, 0xEA, 0x88, 0x6C, 0x0C, 0xDE, 0x07, 0x6E, 0xCF, 0x39, 0x85, 0xB8, 0x04,
                0x9D, 0x35, 0xF3, 0x27, 0x17, 0x7D, 0x48, 0x84, 0x5E, 0x77, 0xC8, 0xB6, 0xC8, 0x89,
                0x28, 0xDB, 0xB6, 0xC2, 0x44, 0xA5, 0x11, 0x1B, 0x52, 0xB4, 0x75
            ]
            .to_vec(),
        );
